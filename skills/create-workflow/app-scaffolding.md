...
When starting a new 0pflow project from scratch, use the `create_app` MCP tool to scaffold the app before creating workflows.
...
Ask for the `service_id` of the existing Tiger Cloud database. You can list available databases with `service_list` if the user needs help finding it.

## Step 2: Create App with create_app Tool

Call the `create_app` MCP tool with the following parameters:
...
Once the database is ready (poll with `service_get` if needed), use `setup_app_schema` to configure the database connection:
