...
// Generates src/crayon/generated/registry.ts with static imports for all workflows, agents, and nodes.
...
interface RegistryEntry {
  ...
  /** Import path relative to src/crayon/generated/registry.ts */
  importPath: string;
}
...
/**
 * Generate src/crayon/generated/registry.ts with static imports for all executables.
...
export function generateRegistry(projectDir: string): string {
  ...
  // Set import paths relative to src/crayon/generated/registry.ts
  ...
  for (const entry of nodes) {
    entry.importPath = `../nodes/${toSourceFilename(nodesDir, entry)}`;
  }
  ...
  // Write the file
  const outDir = path.join(projectDir, "src", "crayon", "generated");
